...
export default function InvoiceTitle({ date, locale, onPrevDay, onNextDay }: Props) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="flex items-center justify-center gap-3">
        ...
        >
          <LuChevronLeft size={18} className={locale === 'ar' ? 'rotate-180' : undefined} />
        </Button>
          ...
          {relativeLabel ? (
            <>
              <span className="text-success">
                {t('header.invoices')} {relativeLabel}:
              </span>
              {' '}{dayName} - {formattedDate}
            </>
          ) : (
            <>
              {t('header.invoices')} {dayName} - {formattedDate}
            </>
          )}
        </h2>
        ...
        >
          <LuChevronRight size={18} className={locale === 'ar' ? 'rotate-180' : undefined} />
        </Button>
      </div>
