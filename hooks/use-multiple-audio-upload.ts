...
interface UseMultipleAudioUploadParams {
  t: TranslateFn;
...
const getAudioUploadEndpoint = () => {
  if (backendBaseUrl) return '/api/admin/upload/audio';
  return '/api/upload/audio';
};

const getAudioPresignedEndpoint = () => {
  if (backendBaseUrl) return '/api/admin/upload/audio/presigned';
...
function resolveAudioContentType(file: File): string | null {
  ...
  if (ALLOWED_AUDIO_TYPES.includes(normalizedType)) return normalizedType;
...
export function useMultipleAudioUpload({
  t,
  onUploaded,
}: UseMultipleAudioUploadParams) {
  ...
  const handleFileSelect = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const validExts = ['mp3', 'wav', 'ogg', 'webm', 'mp4', 'm4a', 'aac'];
      const hasInvalid = files.some((f) => {
        const ext = f.name.split('.').pop()?.toLowerCase() || '';
        return !validExts.includes(ext) && !f.type.startsWith('audio/');
      });
      ...
      const hasOversized = files.some((f) => f.size > MAX_AUDIO_SIZE);
      if (hasOversized) {
      ...
      const calculateOverall = (fileProgress: number) => {
        return Math.round(((completed + fileProgress / 100) / total) * 100);
      };
      ...
      for (const file of files) {
        ...
        if (!resolvedType) {
          toast.error(`${file.name}: ${t('audioInvalidType')}`);
          continue;
        }
        ...
        updateUploadState((prev) => ({
          ...
          overallProgress: calculateOverall(1),
          uploadSpeed: '0 KB/s',
          timeRemaining: '--',
        }));
          ...
          const res = await uploadFileWithProgress({
            ...
            onProgress: (currentFileProgress) => {
              updateUploadState((prev) => ({
                ...prev,
                currentFileProgress,
                overallProgress: calculateOverall(currentFileProgress),
              }));
            },
            onRequestStart: (xhr) => {
        ...
        try {
          ...
          if (isAbortError(error)) {
            toast.info(t('audioUploadCancelled'));
            break;
          }
          toast.error(`${file.name}: ${t('audioUploadFailed')}`);
          updateUploadState((prev) => ({
            ...prev,
            currentFileProgress: 0,
            overallProgress: Math.round((completed / total) * 100),
